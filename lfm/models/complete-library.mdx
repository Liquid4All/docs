...
- Designed for fast inference with [Transformers](/docs/gpu-inference/transformers), [llama.cpp](/docs/on-device/llama-cpp), [vLLM](/docs/gpu-inference/vllm), [MLX](/docs/on-device/mlx), [Ollama](/docs/on-device/ollama), and [LEAP](/docs/on-device/ios/ios-quick-start-guide)
...
- **GGUF** — Best for local CPU/GPU inference on any platform. Use with [llama.cpp](/docs/on-device/llama-cpp), [LM Studio](/docs/on-device/lm-studio), or [Ollama](/docs/on-device/ollama). Append `-GGUF` to any model name.
- **MLX** — Best for Mac users with Apple Silicon. Leverages unified memory for fast inference via [MLX](/docs/on-device/mlx). Browse at [mlx-community](https://huggingface.co/mlx-community/collections?search=LFM).
