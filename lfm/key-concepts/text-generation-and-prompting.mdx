...
Sampling parameters control text generation behavior and balance creativity, determinism, and quality:

Use the recommended values on each model page.
...
| Repetition penalty  (1.0+) | Reduces repetition. `1.0` means no penalty. Values above `1.0` prevent repetition. |
| Output length (`max_tokens` / `max_new_tokens`) | Maximum number of generated tokens |
