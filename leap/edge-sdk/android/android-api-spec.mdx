...
<br/>
### `downloadModel`
Downloads a model from the LEAP Model Library and saves it to the local cache, without loading it into memory.
...
data class GenerationTimeParameters (
    val samplingParameters: SamplingParameters? = null, TODO @davidberrios to confirm default behavior (pull from model config?)
    val numberOfDecodingThreads: Int? = null, TODO @davidberrios to confirm default behavior (if the default is a number, should we just set that here instead of `null`?)
)
```

## `SamplingParameters`
The `SamplingParameters` class contains potential sampling options . It is a data class with the following fields: TODO @davidberrios to confirm default behavior
```kotlin
...
## `Manifest`
The `Manifest` class contains metadata about a downloaded model. It is a data class with the following fields: TODO @davidberrios to provide description/default behavior for each field
```kotlin
data class Manifest(
