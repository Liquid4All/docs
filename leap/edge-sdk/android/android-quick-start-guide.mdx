...
dependencies {
  ...
  implementation("ai.liquid.leap:leap-sdk:0.7.0")
}
```
...
With LeapSDK v0.7.0, you have two options for obtaining and using models:

### Option 1: Manual download and device transfer
...
### Option 2: Direct GGUF download from LeapBundles repo

The LeapSDK now supports direct downloads of GGUF files from the [LeapBundles repository](https://huggingface.co/LiquidAI/LeapBundles/tree/main). You can point directly to a remote JSON manifest, and the SDK will automatically download the necessary GGUF files along with generation parameters.

No manual download or `adb push` is required with this approach - the SDK handles everything automatically.

## Load Model in Code

The `LeapClient.loadModel` suspend function loads a model and returns a model runner instance for invoking the model. This function takes some time to finish as loading the model is a heavy I/O operation, but it is safe to call on the main thread. The function should be executed in a coroutine scope.

### Loading from local bundle file (Option 1)

If you downloaded and pushed a bundle file to your device:
...
### Loading from remote GGUF manifest (Option 2)

To load a model directly from the LeapBundles repository using a remote JSON manifest:

```kotlin
lifecycleScope.launch {
  try {
    // Point to the JSON manifest URL in the LeapBundles repo
    val manifestUrl = "https://huggingface.co/LiquidAI/LeapBundles/raw/main/models/lfm2-1.2b/lfm2-1.2b-20250710-8da4w.json"
    modelRunner = LeapClient.loadModel(manifestUrl)
   }
   catch (e: LeapModelLoadingException) {
     Log.e(TAG, "Failed to load the model. Error message: ${e.message}")
   }
