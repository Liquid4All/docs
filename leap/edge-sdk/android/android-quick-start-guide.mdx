...
The SDK supports two methods for loading models. The automatic approach is recommended for new projects because of superior inference performance and good default generation parameters.
:::

<Tabs>
<TabItem value="automatic" label="Automatic pull with manifests (recommended)" default>

The LeapSDK supports direct downloads of GGUF files from the [LeapBundles repository](https://huggingface.co/LiquidAI/LeapBundles/tree/main). You can point directly to a remote JSON manifest, and the SDK will automatically download the necessary GGUF files along with generation parameters.
...
<TabItem value="manual" label="Manual load with bundles (legacy)">
