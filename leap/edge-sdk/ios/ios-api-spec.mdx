...
| `downloadProgressHandler` | `(_ progress: Double, _speed: Int64) -> Void` | No | `nil` | A callback function to receive the download progress (as a percentage in decimal form between 0 and 1) and speed (in bytes per second). TODO @nickprokopiev is the "required" and "Default" correct for this? I assume a callback is not required, but don't see a default in the code |
...
public struct DownloadedModelManifest {
  ...
  public let local AudioDecoderURL: URL?
