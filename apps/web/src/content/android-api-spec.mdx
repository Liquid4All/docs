...
<Callout type="info">
  All functions listed in this document are safe to be called from the main thread (or UI thread)
  and all callbacks will be run on the main thread if there is no explicit instructions or
  explanations.
</Callout>
...
`isGenerating` value field is true if the generation is still in progress. It is same to check from any threads.
...
Export the whole history into a `JSONArray`. Each element can be interpreted as an `ChatCompletionRequestMessage` instance in OpenAI API schema.
...
Leap Android SDK also has [Gson](https://github.com/google/gson) support. `leap_gson` package should be imported to enable Gson to serialize and deserialize Leap objects. The Gson support has the same behaviors as the existing `org.json` implementation.
...
With a [conversation](#conversation) object, simply calling `Gson.toJson` to convert the chat message history into a JSON string. The return JSON will be an array.

```kotlin
val json = gson.toJson(conversation.history)
```

To deserializing the conversation history from a JSON array, use `LeapGson.messageListTypeToken` as the type hint for Gson.
