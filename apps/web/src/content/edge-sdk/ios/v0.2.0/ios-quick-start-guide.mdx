...
You can also use the model downloader module to download model bundles on-demand in your app, which reduces initial app size and allows for dynamic model selection. See the [Model Downloader API documentation](ios-api-spec#leapmodeldownloader) for details on how to implement on-demand model downloading.
