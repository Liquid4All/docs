...
To use LFM Vision Models with vLLM, install the custom version from the Liquid AI fork:

```bash
pip install git+https://github.com/LiquidAI/vllm.git@lfm-vision-support
```

This fork includes the necessary changes for LFM Vision Model support. Once these changes are merged upstream, you'll be able to use the standard vLLM installation.

### Basic Usage

Initialize a vision model and process images:
...
llm = LLM(model="LiquidAI/LFM2.5-VL-1.6B")
