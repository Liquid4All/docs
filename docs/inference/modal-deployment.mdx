...
## Option 1. Use `vLLM` docker image[​](#option-1-use-vllm-docker-image "Direct link to option-1-use-vllm-docker-image")
...
```
cd modal# deploy LFM2 8B MoE modelmodal deploy deploy-vllm-docker.py# deploy other LFM2 model, MODEL_NAME defaults to LiquidAI/LFM2-8B-A1BMODEL_NAME=LiquidAI/<model-slug> modal deploy deploy-vllm-docker.py
```
...
## Option 2. Use `vLLM` PyPI package[​](#option-2-use-vllm-pypi-package "Direct link to option-2-use-vllm-pypi-package")
...
```
cd modal# deploy LFM2 8B MoE modelmodal deploy deploy-vllm-pypi.py# deploy any LFM2 model, MODEL_NAME defaults to LiquidAI/LFM2-8B-A1BMODEL_NAME=LiquidAI/<model-slug> modal deploy deploy-vllm-pypi.py
```
...
```
# List deployed modelcurl https://<modal-deployment-url>/v1/models# Query the deployed LFM modelcurl https://<modal-deployment-url>/v1/chat/completions \  --json '{  "model": "LiquidAI/LFM2-8B-A1B",  "messages": [    {      "role": "user",      "content": "What is the melting temperature of silver?"    }  ],  "max_tokens": 32,  "temperature": 0}'
```

[Edit this page](https://github.com/Liquid4All/docs/tree/main/lfm/inference/modal-deployment.md)
