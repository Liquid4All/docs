...
<Warning>
LFM Vision Model support in llama.cpp requires a specific version of `llama-cpp-python` that includes changes not yet merged upstream. You must install from a custom source to use vision models. See installation instructions below.
</Warning>
...
To use LFM Vision Models with llama-cpp-python, install the custom version:

```bash
pip install git+https://github.com/LiquidAI/llama-cpp-python.git@lfm-vision-support
```

For GPU support:
```bash
CMAKE_ARGS="-DLLAMA_CUDA=on" pip install git+https://github.com/LiquidAI/llama-cpp-python.git@lfm-vision-support
```

This fork includes the necessary changes for LFM Vision Model support. Once these changes are merged upstream, you'll be able to use the standard `llama-cpp-python` installation.
