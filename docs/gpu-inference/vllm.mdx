...
vLLM offers significantly higher throughput than [Transformers](/docs/gpu-inference/transformers), making it ideal for serving many concurrent requests. However, it requires a CUDA-compatible GPU. For CPU-only environments, consider using [llama.cpp](/docs/on-device/llama-cpp) instead.
