...
<Callout type="info">
  Vision-capable models need the matching mmproj. Bundles include this path as part of their
  metadata; GGUF checkpoints require an `mmproj-*.gguf` sitting beside the model file. When the SDK
  detects the projection, you can attach `ChatMessageContent.image` parts (for tools, vision
  prompts, etc.).
</Callout>
