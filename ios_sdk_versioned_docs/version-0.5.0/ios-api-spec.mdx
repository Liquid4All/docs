...
public class Conversation {
    ...
    // Register a function for the model to invoke
...
- `functionCalls`: Optional field for function call requests generated by the model. See [Function Calling](./function-calling) guide for more details.
...
Currently, only text and image content are supported. Future versions may add support for other content types like audio.

Image data should be in JPEG format. Following extention functions are available to create image content with other image objects:
...
extension ChatMessageContent {
    public static func fromUIImage(_ image: UIImage, compressionQuality: CGFloat = 0.85) throws -> ChatMessageContent
    public static func fromNSImage(_ image: NSImage, compressionQuality: CGFloat = 0.85) throws -> ChatMessageContent
    public static func fromJPEGData(_ jpegData: Data) -> ChatMessageContent
}
```
...
public enum MessageResponse {
    ...
    case functionCalls([LeapFunctionCall])
...
- `functionCalls([LeapFunctionCall])`: Contains function call requestes from the model. It will only be available when some functions are registered to the conversation.
...
- `type` Data type of the parameter. See [Function Calling > Function Definition](./function-calling#function-definition) on the detailed explanation of `LeapFunctionParameterType`.
...
- `arguments` The arguments (parameters) of this call in a map. Values could be strings, numbers, booleans, null, arrays, or dictionaries for objects.
...
public struct GenerationOptions {
    ...
    public var jsonSchemaConstraint: String? // JSON schema constraint for structured output generation.
    public var functionCallParser: (any LeapFunctionCallParserProtocol)? // Function call parser
