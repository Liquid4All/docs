...
<Tabs>
  ...
  <Tab title="Atomic Chat">
    ...
    **Run:**
    ...
    3. Click **Start Server**. It serves the loaded model at `http://localhost:1337` by default; use the port shown in the app.

    The same Integrations screen also includes one-click agent launchers, so you can run a harness right next to the server.
  </Tab>
  <Tab title="MLX">
