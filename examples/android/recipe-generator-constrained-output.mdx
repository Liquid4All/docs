...
- **Type Safety** - Parse AI outputs directly into Kotlin data classes
...
<Accordion title="Model Bundle Deployment">
  This example requires the **LFM2-700M** model bundle to be deployed to your Android device.

  **Step 1: Download the model bundle**

  Download the LFM2-700M bundle from the Leap Model Library (or use your existing bundle file).
  ...
  **Step 2: Push to device via ADB**
  ...
  ```bash
  # Ensure your device is connected and ADB is installed
  adb devices

  # Create the directory on device
  adb shell mkdir -p /data/local/tmp/liquid/

  # Push the model bundle to the device
  adb push LFM2-700M-8da4w_output_8da8w-seq_4096.bundle /data/local/tmp/liquid/

  # Verify the file was transferred
  adb shell ls -lh /data/local/tmp/liquid/
  ```

  **Note:** The exact bundle filename may vary. Adjust the path accordingly.

  **Alternative: Use device storage**

  If you don't have root access to `/data/local/tmp/`, you can push to external storage:

  ```bash
  # Push to Downloads folder
  adb push LFM2-700M-8da4w_output_8da8w-seq_4096.bundle /sdcard/Download/

  # Update the app code to load from this path instead
  ```
</Accordion>
...
<Accordion title="Dependencies Setup">
  ...
  ```kotlin
  dependencies {
      // LeapSDK for constrained generation
      implementation("ai.liquid.leap:leap-sdk:1.0.0")
...
2. **Deploy the model bundle**
   ...
   - Ensure the model is accessible at the correct path
...
4. **Verify model path**
   - Open `MainActivityViewModel.kt`
   - Check that the model path matches where you pushed the bundle
   - Update if necessary
...
class MainActivityViewModel : ViewModel() {
    ...
    fun initializeModel() {
        ...
        model = LeapSDK.loadModelFromBundle(
            path = "/data/local/tmp/liquid/LFM2-700M-8da4w_output_8da8w-seq_4096.bundle"
        )
...
Join the [Liquid AI Discord community](https://discord.gg/DFU3WQeaYD) to:
- Ask questions about constrained generation
- Share your structured output use cases
- Get help with JSON schema design
- Learn about advanced LeapSDK features
- Connect with developers building production AI apps
