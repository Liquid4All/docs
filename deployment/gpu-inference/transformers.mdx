...
See the [Prompting Guide](/lfm/key-concepts/text-generation-and-prompting#text-models) for model-specific recommended settings.
...
output = model.generate(input_ids, do_sample=True, temperature=0.1, top_k=50, top_p=0.1, repetition_penalty=1.05, streamer=streamer, max_new_tokens=512)
```
...
outputs = model.generate(batch, do_sample=True, temperature=0.1, top_k=50, top_p=0.1, repetition_penalty=1.05, max_new_tokens=512)
