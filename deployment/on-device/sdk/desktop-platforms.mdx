...
plugins {
    ...
    id("ai.liquid.leap.nativelibs") version "0.10.9"
}
...
dependencies {
    leapSdkNatives("ai.liquid.leap:leap-sdk-linuxx64:0.10.9:natives@zip")
}
...
- `ai.liquid.leap:leap-sdk-linuxx64:0.10.9:natives@zip`
- `ai.liquid.leap:leap-sdk-linuxarm64:0.10.9:natives@zip`
...
- `ai.liquid.leap:leap-sdk-mingwx64:0.10.9:natives@zip`

<Info>
**Building from macOS or Linux for Windows?** Kotlin/Native does not support cross-compiling to MinGW from a non-Windows host as of 2.3.21 — the build must run on Windows (native or in CI). GitHub Actions `windows-latest` works without extra setup.
</Info>
...
dependencies {
    implementation("ai.liquid.leap:leap-sdk:0.10.9")
    implementation("ai.liquid.leap:leap-ui:0.10.9") // Compose voice widget runs on JVM too
}
```
