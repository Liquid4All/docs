...
<Tabs>
  <Tab title="Swift (iOS / macOS)">
    <Info>
    **New in v0.10.8.** SKIE is now applied to `leap-sdk-openai-client`, matching `LeapSDK` / `LeapModelDownloader` / `LeapUI`. Swift consumers get a real `AsyncSequence`, exhaustive `onEnum(of:)` switching, and the top-level `OpenAiClient(config:)` factory — no more `OpenAiClientKt.` prefix. Kotlin/Native still flattens concrete message types to names such as `ChatMessageUser`. If you need the pre-SKIE manual-collector surface frozen for some reason, pin to `0.10.7`; otherwise use the v0.10.8 surface below.
    </Info>
...
`OpenAiClientConfig` is a Kotlin data class. Kotlin callers can omit defaulted fields; Swift callers must pass all four constructor arguments, as shown above.
...
`ChatMessage` (the OpenAI-client one, distinct from the on-device `ChatMessage` in `LeapSDK` / `LeapModelDownloader`) is a sealed type with three cases — `System`, `User`, `Assistant`. Swift constructs them through the flattened names `ChatMessageSystem`, `ChatMessageUser`, and `ChatMessageAssistant`.
    ...
    final class HybridChatViewModel: ObservableObject {
        ...
        func send(_ text: String, useCloud: Bool) async throws {
            if useCloud {
                ...
                // Kotlin/Native exports the cloud message implementations with flattened
                // names. The on-device ChatMessage remains in LeapModelDownloader.
                let request = ChatCompletionRequest(
                    model: "gpt-4o-mini",
...
The `OpenAiClient(config:)` factory (Kotlin `fun OpenAiClient(config:)`, exposed directly to Swift since v0.10.8) creates an `HttpClient` internally and ties it to the returned client — call `close()` when you're done.
