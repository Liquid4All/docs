...
- **`requestDownloadModel`** — `suspend` fire-and-forget download via WorkManager. The download itself survives app restarts; the call only stages the work request.
- **`requestStopDownload`** — `suspend`; cancels an in-flight background download.
- **`queryStatus`** — `suspend` one-shot status check.
- **`observeDownloadProgress`** — `Flow<ProgressData>` for UI updates during a background download. Not suspend — collect the flow from a coroutine.
