...
- **Web (Wasm, experimental)** — present in the source tree (`leap-ui-demo/web`) but not yet covered by the stable release notes through v0.10.9 — treat as preview.
...
<Tabs>
  ...
  <Tab title="Android / JVM (Gradle)">
    ...
    dependencies {
        ...
        implementation("ai.liquid.leap:leap-ui:0.10.9")
    }
    ```
  ...
  <Tab title="Swift (iOS / macOS)">
    ...
    final class VoiceAssistantViewModel: ObservableObject {
        ...
        private let downloader: ModelDownloader = {
            ...
            return ModelDownloader(config: LeapDownloaderConfig.with(saveDir: modelsDir))
        }()
        ...
        func generateResponse(
            ...
            for try await response in conversation.generateResponse(message: userMessage) {
                ...
                case .chunk, .reasoningChunk, .functionCalls:
                    break
                case .error(let err):
                    // Surface generation failure to the voice store — playback stops gracefully.
                    throw NSError(domain: "LeapVoice", code: 1, userInfo: [NSLocalizedDescriptionKey: err.message])
                }
            }
            return stats
        }
