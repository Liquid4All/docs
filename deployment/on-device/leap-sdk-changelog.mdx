...
- The defaulted convenience init is removed. Swift 6.2 exposes its replacement, **`LeapDownloaderConfig.with(...)`**, carrying the same Kotlin defaults (`saveDir = "leap_models"`, `validateSha256 = true`, `disableSslValidation = false`, `baseUrl = nil`, `connectTimeoutMillis = 30_000`, `socketTimeoutMillis = 60_000`, `requestTimeoutMillis = 600_000`). Swift 6.0/6.1 callers should use the full designated initializer shown in the SDK guides.
...
- `onEnum(of:)` presents the event variants as `.delta`, `.done`, and `.error`. Concrete Kotlin classes remain flattened by Kotlin/Native (for example, Swift constructs messages with `ChatMessageUser`, not `ChatMessage.User`).
- The top-level Kotlin `fun OpenAiClient(config:)` factory is exposed directly to Swift — call sites can drop the `OpenAiClientKt.` prefix and write `OpenAiClient(config: ...)`.
...
iOS / macOS Swift consumers can stay on the Xcode 16 / Swift 6 baseline from v0.10.0. On Swift 6.0/6.1, use the full `LeapDownloaderConfig` designated initializer; its defaulted `.with(...)` factory is exposed only to Swift 6.2+ by the published framework.
